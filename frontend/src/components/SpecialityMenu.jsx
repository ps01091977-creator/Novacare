...
const SpecialityMenu = () => {
    return (
        <div id='speciality' className='flex flex-col items-center gap-4 py-16 text-[#262626]'>
            ...
            <div className='flex sm:justify-center gap-4 pt-5 w-full flex-wrap justify-center pb-4 px-4'>
                {specialityData.map((item, index) => (
                    <Link to={`/doctors/${item.speciality}`} onClick={() => scrollTo(0, 0)} className='flex flex-col items-center gap-2 text-xs sm:text-sm cursor-pointer hover:-translate-y-1 active:scale-95 transition-all duration-300 group w-20 sm:w-24' key={index}>
